...
builder.Services.AddDbContext<UmiDbContext>(options =>
    options.UseInMemoryDatabase("UmiHealthTest"));

// JWT Authentication
var jwtKey = builder.Configuration["Jwt:Key"] ?? "UmiHealthSecretKeyForJWTTokenGeneration-ChangeInProduction";
...
// Health check
app.MapGet("/health", () => new { status = "healthy", timestamp = DateTime.UtcNow, message = "Umi Health Standalone API is running" });
...
app.MapGet("/api/v1/users", async (UmiDbContext db) =>
{
