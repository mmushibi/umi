...
app.MapGet("/api/v1/superadmin/tenants", (Dictionary<string, object> tenantsDb, Services.IAuditService auditService) =>
{
...
app.MapGet("/api/v1/superadmin/tenants/{tenantId}", (string tenantId, Dictionary<string, object> tenantsDb, Services.IAuditService auditService) =>
{
    // TODO: Check user role is super-admin or operations
    if (!tenantsDb.ContainsKey(tenantId))
    {
        auditService.LogSuperAdminAction("super-admin-user", tenantId, "GET_TENANT", "tenants", new() { { "status", "NOT_FOUND" } });
...
app.MapPost("/api/v1/superadmin/tenants", async (HttpRequest request, Dictionary<string, object> tenantsDb, Services.IAuditService auditService) =>
{
    try
    {
        // TODO: Check user role is super-admin or operations
        ...
        auditService.LogSuperAdminAction("super-admin-user", tenantId, "CREATE_TENANT", "tenants", new() { { "name", tenantData["name"] } });
        return Results.Ok(new { success = true, data = tenant, message = "Tenant created" });
    }
    catch (Exception ex)
    {
        auditService.LogSuperAdminAction("super-admin-user", null, "CREATE_TENANT", "tenants", new() { { "error", ex.Message } });
        return Results.BadRequest(new { success = false, message = ex.Message });
    }
});

// Super-admin: Update tenant (e.g., plan, status)
app.MapPut("/api/v1/superadmin/tenants/{tenantId}", async (string tenantId, HttpRequest request, Dictionary<string, object> tenantsDb, Services.IAuditService auditService) =>
{
    try
    {
        // TODO: Check user role is super-admin or operations
    ...
    {
        auditService.LogSuperAdminAction("super-admin-user", tenantId, "UPDATE_TENANT", "tenants", new() { { "error", ex.Message } });
        return Results.BadRequest(new { success = false, message = ex.Message });
    }
});

// Super-admin: Get all users across all tenants
app.MapGet("/api/v1/superadmin/users", (Dictionary<string, object> usersDb, Services.IAuditService auditService) =>
{
    // TODO: Check user role is super-admin or operations
...
app.MapGet("/api/v1/superadmin/tenants/{tenantId}/users", (string tenantId, Dictionary<string, object> usersDb, Services.IAuditService auditService) =>
{
    // TODO: Check user role is super-admin or operations
...
app.MapPut("/api/v1/superadmin/users/{userId}/status", async (string userId, HttpRequest request, Dictionary<string, object> usersDb, Services.IAuditService auditService) =>
{
    try
    {
        // TODO: Check user role is super-admin or operations
        ...
        auditService.LogSuperAdminAction("super-admin-user", existing.tenantId?.ToString(), "UPDATE_USER_STATUS", "users", new() { { "userId", userId }, { "newStatus", newStatus } });
        return Results.Ok(new { success = true, data = updated, message = "User status updated" });
    }
    catch (Exception ex)
    {
        auditService.LogSuperAdminAction("super-admin-user", null, "UPDATE_USER_STATUS", "users", new() { { "error", ex.Message } });
        return Results.BadRequest(new { success = false, message = ex.Message });
    }
});

// Super-admin: Get audit logs
app.MapGet("/api/v1/superadmin/audit", (Services.IAuditService auditService) =>
{
    // TODO: Check user role is super-admin or operations
